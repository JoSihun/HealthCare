...
const GetPagination = ({ pages, size }) => {
    ...
    return (
        <Pagination className="justify-content-center">
            <Pagination.First href={`/support/freeboard/${1}/${size || 20}`} disabled={pages.first}/>
            <Pagination.Prev href={`/support/freeboard/${pages.number}/${size || 20}`} disabled={pages.first}/>

            {/* 5개 미만일 떄 몇개 표시? */}
            <Pagination.Ellipsis href={`/support/freeboard/${ellipsePrev}/${size || 20}`} disabled={pages.first}/> 
            <Pagination.Item href={`/support/freeboard/${paginations.numbers[0]}/${size || 20}`} active={paginations.actives[0]}>{paginations.numbers[0]}</Pagination.Item>
            <Pagination.Item href={`/support/freeboard/${paginations.numbers[1]}/${size || 20}`} active={paginations.actives[1]}>{paginations.numbers[1]}</Pagination.Item>
            <Pagination.Item href={`/support/freeboard/${paginations.numbers[2]}/${size || 20}`} active={paginations.actives[2]}>{paginations.numbers[2]}</Pagination.Item>
            <Pagination.Item href={`/support/freeboard/${paginations.numbers[3]}/${size || 20}`} active={paginations.actives[3]}>{paginations.numbers[3]}</Pagination.Item>
            <Pagination.Item href={`/support/freeboard/${paginations.numbers[4]}/${size || 20}`} active={paginations.actives[4]}>{paginations.numbers[4]}</Pagination.Item>
            <Pagination.Ellipsis href={`/support/freeboard/${ellipseNext}/${size || 20}`} disabled={pages.last}/>
            {/* 5개 미만일 떄 몇개 표시? */}

            <Pagination.Next href={`/support/freeboard/${pages.number+2}/${size || 20}`} disabled={pages.last}/>
            <Pagination.Last href={`/support/freeboard/${pages.totalPages}/${size || 20}`} disabled={pages.last}/>
        </Pagination>
    );
}
